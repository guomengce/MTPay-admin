  ...
  <main class="two-factor-page">
    ...
    <section class="two-factor-card-wrap">
      <form class="two-factor-card" @submit.prevent="submit">
        ...
        <el-input id="admin-two-factor-code" v-model="code" inputmode="numeric" autocomplete="one-time-code" maxlength="6" placeholder="000000" @input="normalizeCode" />
        <el-button type="primary" native-type="submit">驗證並登入</el-button>
        ...
        <p class="two-factor-card__notice"><i class="ri-information-line" />驗證碼每 30 秒更新一次，請勿向任何人透露驗證碼。</p>
      </form>
    </section>
  </main>
</template>
...
function normalizeCode(value:string){code.value=value.replace(/\D/g,'').slice(0,6)}
function submit(){if(!/^\d{6}$/.test(code.value)){ElMessage.warning('請輸入 6 位數字驗證碼');return}ElMessage.info('2FA 驗證介面待接入')}
function back(){void router.replace({name:'Login'})} function help(){ElMessage.info('請聯絡系統管理員重設雙重認證')}
</script>
