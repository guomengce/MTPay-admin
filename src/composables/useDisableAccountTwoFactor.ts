...
export function useDisableAccountTwoFactor(kind: 'admin' | 'agent', refresh: () => Promise<unknown>) {
  ...
  async function disableTwoFactor(row: { id: number; name?: string; company_name?: string; email?: string }) {
    if (twoFactorBusy.value || !Number.isInteger(row.id) || row.id <= 0 || (kind === 'admin' && row.id === 1)) return;
    twoFactorBusy.value = true;
    try {
      try {
        await ElMessageBox.confirm(`確定關閉${kind === 'admin' ? '管理員' : '代理'}「${row.company_name || row.name || row.email || row.id}」的 2FA？關閉後該帳戶登入將不再要求動態驗證碼。`, '關閉 2FA', {
